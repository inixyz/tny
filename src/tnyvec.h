...
struct Data {
  ...
  bool operator==(const Data &rhs) const {
    ...
    case Data::FN: {
      const std::vector<std::string> &lhs_params = std::get<Fn>(val).params;
      const std::vector<std::string> &rhs_params = std::get<Fn>(rhs.val).params;
      const Vec &lhs_ast = std::get<Fn>(val).ast;
      const Vec &rhs_ast = std::get<Fn>(rhs.val).ast;
      if (lhs_params.size() != rhs_params.size() ||
          lhs_ast.size() != rhs_ast.size())
        return false;
      else {
        for (size_t i = 0; i < lhs_params.size(); i++)
          if (lhs_params[i] != rhs_params[i])
            return false;
        for (size_t i = 0; i < lhs_ast.size(); i++)
          if (!(lhs_ast[i] == rhs_ast[i]))
            return false;
        return true;
      }
    }
    ...
    case Data::SYMBOL:
    case Data::STR:
      return std::get<std::string>(val) == std::get<std::string>(rhs.val);
  ...
  operator bool() const {
    ...
    case Data::BUILTIN:
      return (bool)std::get<Builtin>(val);
    case Data::FN:
      return !std::get<Fn>(val).ast.empty();
    case Data::SYMBOL:
    case Data::STR:
...
Data eval(const Data &data, Env &env) {
  ...
  case Data::VEC: {
    ...
    if (vec.empty())
      throw std::runtime_error("can't call empty vector");

    Data fn = eval(vec[0], env);
    vec.erase(vec.begin());
    if (fn.type == Data::BUILTIN)
      return std::get<Builtin>(fn.val)(vec, env);
    else if (fn.type == Data::FN) {
      const std::vector<std::string> &params = std::get<Fn>(fn.val).params;
      ...
      throw std::runtime_error("unexpected data type in call");
  } break;
  default:
    throw std::runtime_error("unknown data type in eval");
  }
}

Data exec(const Vec &ast, Env &env) {
  if (ast.empty())
    throw std::runtime_error("can't exec empty ast");
  Data result;
...
void print(const Data &data, std::ostream &out = std::cout) {
  ...
  case Data::FN: {
    ...
    out << "){";
    for (const auto &expr : fn.ast)
      print(expr);
    out << "}";
  } break;

  case Data::SYMBOL:
    out << std::get<std::string>(data.val);
    break;
  case Data::NUM:
    out << std::get<double>(data.val);
    break;
  case Data::STR:
    out << '"' << std::get<std::string>(data.val) << '"';
    break;
  ...
  default:
    throw std::runtime_error("unknown data type in print");
  }
}
...
Data loop_for(const Vec &args, Env &env) {
  if (args.size() < 4)
    throw std::runtime_error("invalid number of args for loop_for");

  eval(args[0], env); // initial
  Data condition = args[1], increment = args[2];
  Vec ast = args;
  ast.erase(ast.begin(), ast.begin() + 3);
...
Data assign(const Vec &args, Env &env) {
  ...
  static auto add_to_scope = [](const std::string &ident, const Data &data,
                                Env &env) -> Data {
    if (!env.local_scope.empty())
      env.local_scope.top()[ident] = data;
    else
      env.global_scope[ident] = data;
    return data;
  };
  ...
  for (auto it = args.begin(); it != args.end(); it += 2) {
    ...
    result =
        add_to_scope(std::get<std::string>(it->val), eval(*(it + 1), env), env);
  }
  return result;
}

Data subscript(const Vec &args, Env &env) {
  if (args.size() != 2)
    throw std::runtime_error("subscripts must have two arguments");

  Data data = eval(args[0], env);
  Data index_data = eval(args[1], env);

  if (index_data.type != Data::NUM)
    throw std::runtime_error("index for subscript must be number");

  int index = std::get<double>(index_data.val);

  switch (data.type) {
  case Data::STR: {
    const std::string str = std::string{std::get<std::string>(data.val)[index]};
    return {Data::STR, str};
  } break;
  ...
  default:
    throw std::runtime_error("unknown data type for subscript");
  }
}

Data fn(const Vec &args, Env &env) {
  if (args.size() < 2)
    throw std::runtime_error("not enough args for fn");
  ...
  for (const auto &data : std::get<Vec>(params.val))
    if (data.type != Data::SYMBOL)
      ...
      result.params.push_back(std::get<std::string>(data.val));
  for (auto it = args.begin() + 1; it != args.end(); it++)
...
Data vec(const Vec &args, Env &env) {
  if (args.size() < 1)
    throw std::runtime_error("not enough args for vec");

  Vec vec;
  for (auto it = args.begin(); it != args.end(); it++)
    vec.push_back(eval(*it, env));
...
Env::Env() {
  ...
  global_scope["[]"] = {Data::BUILTIN, builtin::subscript};
  global_scope["fn"] = {Data::BUILTIN, builtin::fn};

  global_scope["vec"] = {Data::BUILTIN, builtin::vec};
}
